#!/usr/bin/env python3
"""
Artfical v2.6 - Universal Desktop Explorer
Features:
- On startup opens the current user's Desktop folder in the explorer (cross-platform)
- File explorer (left), editor tabs (center), outline (right), terminal (bottom)
- Auto-save per tab (configurable)
- AST-based outline: imports, prints, defs, classes
- Background installer: installs missing pip packages detected from imports
- Run saved Python files in integrated terminal (QProcess)
- Light/Dark theme toggle

Save as: artfical_v2_6.py
Requires: PySide6
"""
...
APP_NAME = "Artfical v2.6"

# --------------------
# Helpers: Desktop path
# --------------------
def get_desktop_path() -> str:
    """Return the current user's Desktop path cross-platform, fallback to home."""
    home = os.path.expanduser("~")
    # Common desktop names
    candidates = [
        os.path.join(home, "Desktop"),
        os.path.join(home, "Masaüstü"),  # Turkish Windows
...
def outline_from_python(code: str) -> dict:
    ...
    try:
        ...
        return out
    # We want lineno; for prints we inspect Call nodes
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for n in node.names:
            ...
            if isinstance(func, ast.Name) and func.id == "print":
                snippet = "print(...)"
                try:
                    snippet = ast.get_source_segment(code, node) or "print(...)"
                except Exception:
                    pass
...
class PythonHighlighter(QSyntaxHighlighter):
    def __init__(self, doc):
        ...
        self.keyword_compiled = [re.compile(r"\b" + k + r"\b") for k in kws]
    ...
    def highlightBlock(self, text: str):
        ...
        for comp in self.keyword_compiled:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # open user's desktop on startup (universal)
    ...
    def new_tab(self):
        autosave_ms = max(1000, int(float(self.autosave_input.text() or "3")) * 1000)
        tab = EditorTab(None, "", autosave_interval=autosave_ms)
    ...
    def on_outline_clicked(self, item):
        ...
        if not tab:
            return
        block = tab.editor.document().findBlockByLineNumber(max(0, ln - 1))
...
QPlainTextEdit, QLineEdit, QListWidget, QTreeView { background-color: #252526; color: #d4d4d4; }
